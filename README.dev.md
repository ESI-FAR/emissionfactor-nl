# `co2_emission` developer documentation
...
# install co2_emission as an editable package
...
The second is to use `tox`, which can be installed separately (e.g. with `pip install tox`), i.e. not necessarily inside the virtual environment you use for installing `co2_emission`, but then builds the necessary virtual environments itself by simply running:
...
cd $(mktemp -d co2_emission.XXXXXX)
git clone git@github.com:ESI-FAR/co2_emission/co2_emission .
...
Visit
[https://test.pypi.org/project/co2_emission](https://test.pypi.org/project/co2_emission)
and verify that your package was uploaded successfully. Keep the terminal open, we'll need it later.

In a new terminal, without an activated virtual environment or an env directory:

```shell
cd $(mktemp -d co2_emission-test.XXXXXX)
...
--extra-index-url https://pypi.org/simple co2_emission
```
...
Don't forget to also make a [release on GitHub](https://github.com/ESI-FAR/co2_emission/co2_emission/releases/new).GitHub-Zenodo integration will also trigger Zenodo into making a snapshot of your repository and sticking a DOI on it.